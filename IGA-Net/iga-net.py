...
class DeepGCN(torch.nn.Module):
    ...
    def forward(self, inputs):
        ...
        recon_feat2, new_spix_indices = self.make_clusters(inputs, self.p2sp_index_, self.invisible, self.init,
                                                           self.cir,
                                                           self.spixels_h, self.spixels_w, self.device)

        # attention

        x = self.stem(recon_feat2) + self.pos_embed  # 本来是inputs直接输入stem
