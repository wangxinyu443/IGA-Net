...
def SpixelFeature(feat, init_index, max_spixels=50, invisible_p=None):
    ...
    :param init_index:   each superpixel with corresponding pixel coordinates
    ...
    init_x = torch.as_tensor(init_x).type(torch.long)
    init_y = torch.as_tensor(init_y).type(torch.long)
...
def Passoc_Nspixel(spixel_init, num_spixels_w, num_spixs):
  """
  calculate each pixel with corresponding 9 neighborhood superpixel ids and whether is visible
  ...
  :return: p2sp_index_: the index of superpixel of a pixel H*W*9
...
def SpixelFeature2(pixel_features, pixel_assoc, cir_index, invisible, num_spixels_h, num_spixels_w):
    '''
    calculate superpixel feature according to the similarity matrix between pixel and spixel
    :param pixel_features: B*C*H*W
    ...
    cir_x = cir_x.to(device=torch.device("cuda"))
    cir_y = cir_y.to(device=torch.device("cuda"))
    cir_z = cir_z.to(device=torch.device("cuda"))
    cir_l = cir_l.to(device=torch.device("cuda"))
    invisible = invisible.to(device=torch.device("cuda"))
...
def compute_assignments(spixel_feat, pixel_features, p2sp_index_, invisible, device):
...
# 迭代
# p2sp_index:(224,224,9):the index of superpixel of a pixel   H*W*9
# invisible: (224,224,9) whether the surrounding superpixel is available   H*W*9
# pixel_assoc: (1,9,224,224)
# spixel_feat: (1,3,196)
def exec_iter(spixel_feat, trans_features, cir_index, p2sp_index_, invisible, num_spixels_h, num_spixels_w, device):
    ...
    pixel_assoc = \
        compute_assignments(spixel_feat, trans_features, p2sp_index_, invisible, device)# (1,9,224,224)
    # t2 = time.time()
...
def compute_final_spixel_labels(final_pixel_assoc, p2sp_index, num_spixels_h, num_spixels_w):
    ...
    def RelToAbsIndex(rel_label, p2sp_index, num_spixels_h=1, num_spixels_w=1):
        """
        :param rel_label: B*H*W  the position(0-8) of the most similar superpixel of  each pixel
        :param p2sp_index: B*H*W*9  ndarray
        :param num_spixels_h:
        :param num_spixels_w:
...
def Semar(new_spixel_feat, new_spix_indices):
    """
    convert superpixel feature to pixel via hard threshold
