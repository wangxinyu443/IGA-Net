...
class SPIN(nn.Module):
    def __init__(self, colors=3, dim=40, block_num=8, heads=1, qk_dim=24, mlp_dim=72,
        ...
        for i in range(block_num):
            self.blocks.append(
                Block(dim=dim, layer_num=2, stoken_size=[stoken_size[i], stoken_size[i]], heads=heads, qk_dim=qk_dim,
                      mlp_dim=mlp_dim))
            self.mid_convs.append(nn.Conv2d(dim, dim, 3, 1, 1))

        if upscale == 4:
            self.upconv1 = nn.Conv2d(dim, dim * 4, 3, 1, 1, bias=True)  # (B,C,H,W)-->(B,C*4,H,W)
            self.upconv2 = nn.Conv2d(dim, dim * 4, 3, 1, 1, bias=True)
            self.pixel_shuffle = nn.PixelShuffle(2) # (B,C,H,W)-->(B,C/4,H*2,W*2)
    ...
    def forward(self, x):
        ...
        if self.upscale == 4:   # (B,C,H,W)-->(B,C,H*4,W*4)
            out = self.lrelu(self.pixel_shuffle(self.upconv1(x)))
            out = self.lrelu(self.pixel_shuffle(self.upconv2(out))) # (B,C,H,W)-->(B,C/4,H*4,W*4)
        else:
            out = self.lrelu(self.pixel_shuffle(self.upconv(x)))

        out = base + self.last_conv(out)
        return out * 255.   # (B, C, H*upscale, W*upscale)
...
def build_head(output_filters, bias_init):
    ...
    for layer in head:
        if isinstance(layer, nn.Conv2d):
            init.normal_(layer.weight, mean=mean, std=stddev)

    # 使用指定的偏置初始化器初始化最后一层的偏置
    if bias_init is not None:
        init.constant_(head[-1].bias, bias_init)
...
def ssn_iter(pixel_features, stoken_size=[16, 16], n_iter=2):
    ...
    out_spixel_init, feat_spixel_init, spixels_h, spixels_w = \
        transform_and_get_spixel_init(num_spixels, [224, 224])
    ...
    with torch.no_grad():
        for k in range(n_iter):
            if k < n_iter - 1:
                ...
                dist_matrix = Passoc(pixel_features, spixel_features, p2sp_index_, invisible, device)
...
# Superpixel Cross Attention (SPCA)
# 而SPCA模块则捕获远程像素之间的依赖关系和相互作用。
# 首先将信息从像素传播到超像素，然后通过交叉注意力机制将聚合的信息分发到像素。
class SPInterAttModule(nn.Module):
    ...
    def forward(self, x, affinity_matrix, num_spixels):
        ...
        # stoken projection
        stoken = self.sp(stoken).permute(0, 2, 1).reshape(B, self.num_heads, self.qk_dim // self.num_heads,
                                                          num_spixels)  # B, H, C, hh*ww QS
...
# Intra-Superpixel Attention (ISPA) module
# ISPA模块捕获每个超像素内像素的依赖关系和相互作用
class SPIntraAttModule(nn.Module):
    ...
    def forward(self, x, affinity_matrix, num_spixels):
        ...
        q_sp_pixels = torch.gather(q.reshape(B, 1, -1, H * W).expand(-1, num_spixels, -1, -1), -1,
                                   indices.unsqueeze(2).expand(-1, -1, self.qk_dim, -1))  # B, K, C, topk
        k_sp_pixels = torch.gather(k.reshape(B, 1, -1, H * W).expand(-1, num_spixels, -1, -1), -1,
                                   indices.unsqueeze(2).expand(-1, -1, self.qk_dim, -1))  # B, K, C, topk
        v_sp_pixels = torch.gather(v.reshape(B, 1, -1, H * W).expand(-1, num_spixels, -1, -1), -1,
                                   indices.unsqueeze(2).expand(-1, -1, self.dim, -1))  # B, K, C, topk

        q_sp_pixels, k_sp_pixels, v_sp_pixels = map(
            lambda t: rearrange(t, 'b k (h c) t -> b k h t c', h=self.num_heads), \
...
# 为了增强局部区域内像素之间的交互作用，在ISPA和SPCA模块之后使用了一个局部关注模块，该模块使用基于窗口的关注
